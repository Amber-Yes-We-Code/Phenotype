...
class mnistEncoder(nn.Module):
    def __init__(
        ...
        self.fc3 = nn.Linear(64, latent_dim + eta_dim)
...
class CausalMultimodal(nn.Module):
    def __init__(
        ...
        self.num_latent = sum(latent_dims)
    ...
    def forward(self, args, epoch, inputs: List[torch.Tensor]):
        ...
        # sparsity_loss = torch.norm(A_xy, p=1) # - 0.01*torch.norm(Sigma_xy, p='fro')
        # sparsity_loss = torch.mean(torch.abs(matrix)) # - torch.var(matrix)
