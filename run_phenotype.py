...
from scenario import variant_mnist, phenotype
...
def parse_args():
    ...
    # Experiment Settings
    parser.add_argument('--exp_choice', type=str, default='phenotype', choices=['synthetic', 'mnist', 'phenotype'])
    parser.add_argument('--case_id', type=str, default='phenotype')
    parser.add_argument('--seed', type=int, default=random.randint(0, 2**32 - 1))
    parser.add_argument('--loaded_data_path', type=str, default='./dataset/phenotype/tabular.csv', 
    ...
    parser.add_argument('--eta_dims', type=int, nargs='+', default=[1, 1, 1])
    parser.add_argument('--latent_dims', type=int, default=[4, 4, 4])
    parser.add_argument('--input_types', type=str, nargs='+', default=['m1', 'm2', 'm3'])
    parser.add_argument('--input_order', type=str, nargs='+', default=['m1', 'm2', 'm3'])
    parser.add_argument('--hidden_dims', type=int, nargs='+', default=[16, 16, 16])
    parser.add_argument('--shared_dims', type=int, nargs='+', default=[1, 1, 1])

    # Training Parameters
    parser.add_argument('--lr', type=float, default=2e-6)
    parser.add_argument('--num_epochs', type=int, default=int(1e3))
    ...
    parser.add_argument('--plot_itr', type=int, default=2000)
...
def main(selected_keys):
    ...
    DATA_SCENARIOS = {
        ...
        "phenotype": lambda: phenotype(args) 
    }
    ...
    args.data_source = f'./dataset/phenotype'
    args.result_path = f'./result/{args.case_id}/{args.seed}'
    args.true_matrix = f'{args.data_source}/true_matrix.pt'
    ...
    representation = extract_representation(model, all_data, input_dims, args)

    # Step 3: Update data
    logging.info("Updating data with new representations...")
    updated_data_path = update_representation(args, representation)
    
    # Step 4: Perform causal analysis
    logging.info("Performing causal analysis...")
    perform_causal_discovery(args, updated_data_path, selected_keys)
